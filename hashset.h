...
typedef struct {
  ...
  int loglength;
  int numBuckets;
  int (*HashSetHashFunction)(const void* elemAddr, int numBuckets);
  int (*HashSetCompareFunction)(const void* elemAddr1, const void* elemAddr2);
  void (*HashSetFreeFunction)(void* elemAddr);
} hashset;
